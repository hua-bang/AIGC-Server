...
export class NotionService {
  ...
  async saveUrlSummary({ title, link, summary, databaseId }: UrlSummaryData): Promise<string> {
    try {
      // 创建 Notion 页面
      const response = await this.notion.pages.create({
        parent: { database_id: databaseId },
      ...
      console.error("Error saving URL summary to Notion:", error);
      throw error;
    }
  }
} 